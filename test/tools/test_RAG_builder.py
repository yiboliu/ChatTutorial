...
from src.tools.RAG_builder import (
    build_rag, add_text_chunk_to_db, initiate_storage,
    chunk_text, process_chunk, semantic_search
)
...
class TestRAGBuilder(unittest.TestCase):
    ...
    def test_build_rag(self, mock_chunk_text, mock_extract_content, mock_initiate_storage):
    ...
    def test_add_text_chunk_to_db(self, mock_connect):
        ...
        mock_connect.assert_called_once_with(host='weaviate', port=8080, grpc_port=50051)
    ...
    def test_chunk_text(self, mock_process_chunk, mock_sent_tokenize):
        mock_sent_tokenize.return_value = ['Sentence 1.', 'Sentence 2.', 'Sentence 3.', 'Sentence 4.']
    ...
    def test_semantic_search(self, mock_sentence_transformer, mock_connect):
        ...
        mock_response.objects = [
            MagicMock(vector={'default': np.array([1, 0, 0])}, properties={'content': 'Content 1'}),
            MagicMock(vector={'default': np.array([0, 1, 0])}, properties={'content': 'Content 2'}),
            MagicMock(vector={'default': np.array([0, 0, 1])}, properties={'content': 'Content 3'})
        ]
        ...
        mock_sentence_transformer.assert_called_once_with('sentence-transformers/multi-qa-MiniLM-L6-cos-v1')
