...
class TestApp(unittest.TestCase):
    ...
    def test_perform_operation(self, mock_uuid4, mock_mkdtemp, mock_build_rag, mock_connect_to_local):
        # Set up mocks
        ...
        mock_connect_to_local.assert_called_once_with(host='weaviate', port=8080, grpc_port=50051)
        ...
        with self.app.session_transaction() as session:
            ...
            self.assertTrue(any('Operation performed successfully' in message for _, message in flash_messages))
    ...
    def test_cleanup(self, mock_exists, mock_rmtree):
        ...
        with app.app_context():
            user_session = UserSession.query.filter_by(session_id='test_session').first()
            self.assertIsNone(user_session)

        # Check if flash message was set
        with self.app.session_transaction() as session:
            flash_messages = session.get('_flashes', [])
            self.assertTrue(any('Temporary data cleaned up' in message for _, message in flash_messages))
